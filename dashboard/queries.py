...
def products(selected_retailers: list[str] | None = None) -> pd.DataFrame:
    where_clause = "where source in %(retailers)s" if selected_retailers else ""
    sql = f"""
    ...
    """
    params = {"retailers": tuple(selected_retailers)} if selected_retailers else {}
...
def availability_by_product(
    ...
    sql = f"""
        ...
        {"and source in %(retailers)s" if selected_retailers else ""}
        {"and product_key in %(products)s" if selected_products else ""}
        group by 1, 2, 3, 4
        order by avg_availability_pct asc
    """
    params: dict = {"start_date": start_date, "end_date": end_date}
    if selected_retailers:
        params["retailers"] = tuple(selected_retailers)
    if selected_products:
        params["products"] = tuple(selected_products)
    return run_query(sql, params)
...
def state_changes(start_date, end_date, selected_retailers) -> pd.DataFrame:
    sql = f"""
        ...
        {"and fsc.source in %(retailers)s" if selected_retailers else ""}
